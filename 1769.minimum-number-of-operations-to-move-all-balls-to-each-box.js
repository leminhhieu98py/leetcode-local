...
var minOperations = function (boxes) {
  const boxesInArr = boxes.split('');
  const resultArr = boxesInArr.reduce((acc, curr, currIndex) => {
    const minimumOperations = boxesInArr.reduce((_acc, _curr, _currIndex) => {
      let steps = 0;
      if (_curr === '1') {
        steps = Math.abs(_currIndex - currIndex);
      }

      return _acc + steps;
    }, 0);

    acc.push(minimumOperations);
